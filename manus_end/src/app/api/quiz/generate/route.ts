...
export async function POST(request: Request) {
  ...
  for (const proto of selected) {
    ...
    if (openai) {
      try {
        const systemPrompt = `
You are an EMS educator. Generate ONE multiple-choice question (exactly four options) testing knowledge of the protocol titled "${proto.title}". Respond with ONLY valid JSON matching this schema:
{
  "id": string,
  "questionText": string,
  "questionType": "multiple-choice",
