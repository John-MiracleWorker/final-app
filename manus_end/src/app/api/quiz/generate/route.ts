...
export default function QuizPage() {
  ...
  const startQuiz = async () => {
    if (!selectedCategories.length) { setError('Select a category'); return; }
    setError(null);
    setIsLoading(true);
    try {
    ...
    } catch { setError('Quiz generation error'); }
    setIsLoading(false);
  };
  ...
  const nextStep = () => {
    setShowFeedback(false); setSelectedOption('');
  ...
  if (quizState === 'setup') {
    ...
    screen = (
      <div className="p-6 max-w-md mx-auto text-white">
        ...
        {showFeedback ? (
          <><p className="mb-2">{q.explanation}</p><Button onClick={nextStep} className="w-full">{currentIndex + 1 < questions.length ? 'Next' : 'Finish'}</Button></>
        ) : (
